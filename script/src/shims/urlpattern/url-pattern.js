...
const path_to_regex_modified_js_1 = require("./path-to-regex-modified.js");
const url_utils_js_1 = require("./url-utils.js");
const url_pattern_parser_js_1 = require("./url-pattern-parser.js");
...
function extractValues(url, baseURL) {
    ...
    return {
        ...
        pathname: o.pathname,
        search: o.search != ""
            ? o.search.substring(1, o.search.length)
            : undefined,
...
function applyInit(o, init, isPattern) {
    ...
    if (typeof init.protocol === "string") {
        o.protocol = (0, url_utils_js_1.canonicalizeProtocol)(init.protocol, isPattern);
    }
    if (typeof init.username === "string") {
        o.username = (0, url_utils_js_1.canonicalizeUsername)(init.username, isPattern);
    }
    if (typeof init.password === "string") {
        o.password = (0, url_utils_js_1.canonicalizePassword)(init.password, isPattern);
    }
    if (typeof init.hostname === "string") {
        o.hostname = (0, url_utils_js_1.canonicalizeHostname)(init.hostname, isPattern);
    }
    if (typeof init.port === "string") {
        o.port = (0, url_utils_js_1.canonicalizePort)(init.port, o.protocol, isPattern);
    }
    if (typeof init.pathname === "string") {
        o.pathname = init.pathname;
        if (baseURL && !(0, url_utils_js_1.isAbsolutePathname)(o.pathname, isPattern)) {
            ...
            if (slashIndex >= 0) {
                ...
                o.pathname = baseURL.pathname.substring(0, slashIndex + 1) +
                    o.pathname;
            }
        }
        o.pathname = (0, url_utils_js_1.canonicalizePathname)(o.pathname, o.protocol, isPattern);
    }
    if (typeof init.search === "string") {
        o.search = (0, url_utils_js_1.canonicalizeSearch)(init.search, isPattern);
    }
    if (typeof init.hash === "string") {
        o.hash = (0, url_utils_js_1.canonicalizeHash)(init.hash, isPattern);
    }
    return o;
}
...
function tokensToPattern(tokens, options) {
    ...
    for (let i = 0; i < tokens.length; ++i) {
        ...
        const optionsPrefixes = options.prefixes !== undefined
            ? options.prefixes
            : "./";
        let needsGrouping = token.suffix !== "" ||
            (token.prefix !== "" &&
...
class URLPattern {
    constructor(init = {}, baseURL) {
        ...
        Object.defineProperty(this, "component_pattern", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {}
        ...
        try {
            ...
            if (typeof init === "string") {
                const parser = new url_pattern_parser_js_1.Parser(init);
                parser.parse();
                init = parser.result;
                if (baseURL) {
            ...
            if ((0, url_utils_js_1.defaultPortForProtocol)(this.pattern.protocol) ===
                this.pattern.port) {
                this.pattern.port = "";
            }
            let component;
            ...
            for (component of COMPONENTS) {
                ...
                switch (component) {
                    case "protocol":
                        Object.assign(options, url_utils_js_1.DEFAULT_OPTIONS);
                        options.encodePart = url_utils_js_1.protocolEncodeCallback;
                        break;
                    case "username":
                        Object.assign(options, url_utils_js_1.DEFAULT_OPTIONS);
                        options.encodePart = url_utils_js_1.usernameEncodeCallback;
                        break;
                    case "password":
                        Object.assign(options, url_utils_js_1.DEFAULT_OPTIONS);
                        options.encodePart = url_utils_js_1.passwordEncodeCallback;
                        break;
                    case "hostname":
                        Object.assign(options, url_utils_js_1.HOSTNAME_OPTIONS);
                        if ((0, url_utils_js_1.treatAsIPv6Hostname)(pattern)) {
                            options.encodePart = url_utils_js_1.ipv6HostnameEncodeCallback;
                        }
                        else {
                            options.encodePart = url_utils_js_1.hostnameEncodeCallback;
                        }
                        break;
                    case "port":
                        Object.assign(options, url_utils_js_1.DEFAULT_OPTIONS);
                        options.encodePart = url_utils_js_1.portEncodeCallback;
                        break;
                    case "pathname":
                        if ((0, url_utils_js_1.isSpecialScheme)(this.regexp.protocol)) {
                            Object.assign(options, url_utils_js_1.PATHNAME_OPTIONS);
                            options.encodePart =
                                url_utils_js_1.standardURLPathnameEncodeCallback;
                        }
                        else {
                            Object.assign(options, url_utils_js_1.DEFAULT_OPTIONS);
                            options.encodePart = url_utils_js_1.pathURLPathnameEncodeCallback;
                        }
                        break;
                    case "search":
                        Object.assign(options, url_utils_js_1.DEFAULT_OPTIONS);
                        options.encodePart = url_utils_js_1.searchEncodeCallback;
                        break;
                    case "hash":
                        Object.assign(options, url_utils_js_1.DEFAULT_OPTIONS);
                        options.encodePart = url_utils_js_1.hashEncodeCallback;
                        break;
                }
                try {
                    const tokens = (0, path_to_regex_modified_js_1.parse)(pattern, options);
                    this.regexp[component] = (0, path_to_regex_modified_js_1.tokensToRegexp)(tokens, this.keys[component], options);
